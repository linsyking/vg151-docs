...
.. figure:: /_static/l9/stack_vs_queue.webp
  :scale: 100
...
.. code-block:: cpp
  ...
  void move_op(char op1, std::queue<char> &output, std::stack<char> &ops) {
      ...
      while (!ops.empty() && !is_greater(ops.top(), op1)) {
          output.push(ops.top());
          ops.pop();
      }
  }
...
Make sure you have already installed ``flex`` and ``bison``. If not, you can install them by:
...
It is a calculator that supports basic arithmetic operations, parentheses, and some functions. You can also assign a value to a variable and use it later.
